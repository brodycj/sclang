...
struct MiddleSCWrapper {
    ...
    // XXX TBD RECONSIDER KEEPING WHAT STATE TO KEEP HERE - ??? ??? ???
...
struct SCLinkageInfo {
    ...
    // - inner inner middle wrapper - strong reference
    // - XXX TBD XXX XXX middle wrapper - XXX TBD ??? ??? - strong reference
    ...
    fn drop(&mut self) {
        ...
        let inner_sc_linkage_ref = RcRef::new(SCLinkageInfo {
            ...
            linkage2: (
                ...
                linked_sc_data_storage_ref2.inner_inner_middle_sc_wrapper_ref.read().unwrap().upgrade(),
                linked_sc_data_storage_ref2.inner_inner_middle_sc_wrapper_ref.read().unwrap().upgrade(),
            ),
        });
...
impl OuterSCWrapper {
    fn create_with_sc_data(text1: &str, text2: &str, link1: Option<MiddleSCWrapperRcRef>, link2: Option<MiddleSCWrapperRcRef>) -> OuterSCWrapperRcRef {
        // let middle_sc_wrapper_ref = MiddleCellWrapper::create_with_inner_sc_data(text1, text2, link1, link2);
        let inner_inner_sc_wrapper_ref = MiddleSCWrapper::create_with_inner_sc_data(text1, text2, link1, link2);
        // let middle_sc_wrapper_ref = inner_inner_sc_wrapper_ref;
        let middle_sc_wrapper_ref = MiddleSCWrapper::create_with_inner_inner_wrapper(inner_inner_sc_wrapper_ref);
    ...
    fn create_with_outer_middle_wrapper(middle_sc_wrapper_ref: MiddleSCWrapperRcRef) -> OuterSCWrapperRcRef {
        let outer_wrapper_ref = RcRef::new(OuterSCWrapper {
            outer_middle_sc_wrapper: RwCell::new(middle_sc_wrapper_ref.clone()),
            inner_sc_data_storage: middle_sc_wrapper_ref.sc_data_storage.clone(),
        });
    ...
    fn ref_middle_sc_wrapper_ref(middle_sc_wrapper_ref: MiddleSCWrapperRcRef) -> OuterSCWrapperRcRef {
        ...
        if my_outer_wrapper_ref.is_none() {
            OuterSCWrapper::create_with_outer_middle_wrapper(middle_sc_wrapper_ref)
        } else {
...
impl MiddleSCWrapper {
    fn create_with_inner_sc_data(
        ...
        *inner_sc_data_storage.inner_inner_middle_sc_wrapper_ref.write().unwrap() = RcRef::downgrade(&middle_cw_ref.clone());

        return middle_cw_ref;
    }

    // XXX TODO RECONSIDER & FIX NAMING OF THIS FN
    fn create_with_previous_middle_sc_wrapper_data(
        ...
        let inner_inner_middle_wrapper = inner_sc_data_storage_ref.inner_inner_middle_sc_wrapper_ref.read().unwrap().upgrade();
        ...
        *cell_linkage_weak_ref_writer = RcRef::downgrade(&cell_linkage_strong_ref.read().unwrap().clone().unwrap());
        ...
        let middle_wrapper_ref = RcRef::new(MiddleSCWrapper {
            ...
            // XXX TODO CLEANUP THIS CODE HERE:
    ...
    fn create_with_inner_inner_wrapper(inner_inner_wrapper_ref: MiddleSCWrapperRcRef) -> MiddleSCWrapperRcRef {
        // XXX TODO REMOVE EXTRA REF & EXTRA CLONE HERE IF POSSIBLE
        let inner_inner_wrapper = inner_inner_wrapper_ref.clone();

        // XXX TBD EXTRA CLONE ??? ???
        let inner_sc_data_storage = inner_inner_wrapper.clone().sc_data_storage.clone();

        let cell_linkage_strong_ref = RwCell::new(inner_sc_data_storage.peer_sc_linkage_ref.read().unwrap().upgrade());
        ...
        let middle_wrapper_ref = RcRef::new(MiddleSCWrapper {
            ...
            inner_inner_middle_wrapper: RwLock::new(Some(
                inner_sc_data_storage
                    .clone()
                    .inner_inner_middle_sc_wrapper_ref
                    .read()
                    .unwrap()
                    .upgrade()
                    .unwrap(),
...
impl InnerSCDataStorage {
    fn create_with_inner_text_fields(text1: &str, text2: &str) -> InnerSCDataStorageRcRef {
        RcRef::new(InnerSCDataStorage {
            ...
            inner_inner_middle_sc_wrapper_ref: RwCell::new(WeakRef::new()),
        })
    }
